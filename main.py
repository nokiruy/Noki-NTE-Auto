...
from 异环任务 import (店长特供, 超强音, 异环钓鱼, 速切宏战斗线程, 速切宏战斗线程2,
                      自动剧情, 自动按F, 鼠标快速打开esc界面, 赛车, 暗黑赛车活动挂机,
                       弹起所有按键, 呗果, 格斗争霸赛活动挂机,徊影憧憬活动挂机,九百九十九活动刷,)
        ...
        if 异环脚本运行:
            ...
            if 异环脚本运行:
                ...
                def 创建只选择点击方式任务窗口(父容器, 任务名称, 提示文本):
                    ...
                    def 启动任务():
                        if (
                            九百九十九路线变量 is not None
                            and 九百九十九路线变量.get() == "后期-真红刷纽扣"
                            and 异环游戏静音变量.get()
                        ):
                            messagebox.showwarning(
                                "无法启动任务",
                                "九百九十九夜的后期任务需要通过游戏声音进行判定。\n\n"
                                "请先取消勾选主界面的“运行任务时游戏静音”，再启动任务。",
                            )
                            return
